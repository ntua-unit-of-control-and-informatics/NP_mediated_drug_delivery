...
Rat_model <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dM_lu_is <- (1-sigma_lu)*QL_lu*C_lu_cap  - k_lu_in *M_lu_is + k_lu_out *M_lu_cell - 
                 M_lu_is*pc_endo*Q_pc_lu + pc_exo*M_lu_pc -QL_lu*C_lu_is
    dM_lu_cell <- k_lu_in *M_lu_is - k_lu_out *M_lu_cell
    dM_lu_pc <-  M_lu_is*pc_endo*Q_pc_lu - pc_exo*M_lu_pc
    
    #Rest of the body
    dM_rob_cap <-  Q_total*C_art - (Q_total-QL_rob)*C_rob_cap - (1-sigma_rob)*QL_rob*C_rob_cap -
                  M_rob_cap*pc_endo*Q_pc_sinusoids+
                  pc_exo*M_rob_pc*Q_pc_sinusoids/(Q_pc_interstitial+Q_pc_sinusoids)
    dM_rob_is <- (1-sigma_rob)*QL_rob*C_rob_cap -M_rob_is*pc_endo*Q_pc_interstitial - 
                     QL_rob*C_rob_is- (CLE*M_rob_is)/(Km+M_rob_is)+
                    pc_exo*M_rob_pc*Q_pc_interstitial/(Q_pc_interstitial+Q_pc_sinusoids)
    dM_rob_cell <- 0
    dM_rob_pc <-  M_rob_cap*pc_endo*Q_pc_sinusoids+M_rob_is*pc_endo*Q_pc_interstitial -pc_exo*M_rob_pc
...
obj_func <- function(x, dose, df, pars, metric = "AAFE"){
  ...
  parms <- c("rob_pore_size" = exp(x[1]), "lung_pore_size" = exp(x[2]),
             ...
             "pc_endo" =  exp(x[5]) , "pc_exo" =  exp(x[6]),
             "Q_pc_sinusoids" =  exp(x[7]),"Q_pc_interstitial" =  exp(x[8]),
             "Q_pc_lu" =  exp(x[9]),
...
optimization<- nloptr::nloptr(x0 = x0,
                              eval_f = obj_func,
                              lb	=  c(log(6.55),log(6.55),-10,-10,-5,-30,-10,-10,-10),
