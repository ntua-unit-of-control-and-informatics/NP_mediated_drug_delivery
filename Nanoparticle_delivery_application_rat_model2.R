...
Rat_model <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    phagocytised_rob = phago_uptake_rate*(1-(M_rob_phag/M_rob_max))
    k_de <- 10^(-18)
...
obj_func <- function(x, dose, df, pars, metric = "AAFE"){
  ...
  parms <- c("rob_pore_size" = exp(x[1]),  "CLE" = exp(x[2]),
             ...
             "M_lu_max" =  exp(x[4]) , "M_rob_max" =  exp(x[5]),
             "k_rob_in" =  exp(x[6]) , "k_rob_out" =  exp(x[7]),
             "lung_pore_size" = exp(x[8]),
...
x0 <-  c(log(6.5),1,1,1,1,log(2),log(0.1),log(6.5))
set.seed(1221312)
optimization<- nloptr::nloptr(x0 = x0,
                              eval_f = obj_func,
                              lb	=  c(log(100),-20,-20,-20,-20,log(0.001),log(0.001),log(6.5)),
                              ub =   c(log(2500),20,20,20,20,log(20),log(200),log(20) ),
                              opts = opts,
                              dose = dose,
                              df = df,
                              metric = "AAFE",
...
parms <- c("rob_pore_size" = exp(optimization$solution[1]), 
           ...
           "M_rob_max" =  exp(optimization$solution[5]),
           "k_rob_in" =  exp(optimization$solution[6]) , 
           "k_rob_out" =  exp(optimization$solution[7]),
           "lung_pore_size" = exp(optimization$solution[8]),
