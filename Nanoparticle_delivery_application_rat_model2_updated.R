...
create.inits <- function( dose){
  ...
  M_rob_cell <- 0; M_rob_phag_sinusoids <- 0;M_rob_phag_interstital<- 0
  ...
  return(c( "M_lu_cap" = M_lu_cap, "M_lu_is"=M_lu_is,"M_lu_cell" = M_lu_cell,
            ...
            "M_rob_phag_sinusoids" = M_rob_phag_sinusoids,
            "M_rob_phag_interstital"=M_rob_phag_interstital,
...
Rat_model <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    phagocytised_lu = phago_uptake_rate*(1-(M_lu_phag/M_lu_max))
    phagocytised_rob_sinusoids = phago_uptake_rate*(1-(M_rob_phag_sinusoids/M_rob_sinusoids_max))
    phagocytised_rob_interstitial = phago_uptake_rate*(1-(M_rob_phag_interstital/M_rob_interstitial_max))
    ...
    dM_lu_is <- (1-sigma_lu)*QL_lu*C_lu_cap  - k_lu_in *M_lu_is + k_lu_out *M_lu_cell - 
                   M_lu_is*phagocytised_lu - QL_lu*C_lu_is
    dM_lu_cell <- k_lu_in *M_lu_is - k_lu_out *M_lu_cell
    dM_lu_phag <-  M_lu_is*phagocytised_lu 
    
    #Rest of the body
    dM_rob_cap <-  Q_total*C_art - (Q_total-QL_rob)*C_rob_cap - (1-sigma_rob)*QL_rob*C_rob_cap -
                    M_rob_cap*phagocytised_rob_sinusoids
    dM_rob_is <- (1-sigma_rob)*QL_rob*C_rob_cap  - k_rob_in *M_rob_is + k_rob_out *M_rob_cell - 
                   M_rob_is*phagocytised_rob_interstitial - QL_rob*C_rob_is
    dM_rob_cell <- k_rob_in *M_rob_is - k_rob_out *M_rob_cell - CLE*M_rob_cell
    dM_rob_phag_sinusoids <-  M_rob_cap*phagocytised_rob_sinusoids 
    dM_rob_phag_interstital <-  M_rob_is*phagocytised_rob_interstitial 
    ...
    # Excreta
    dM_excreta <- CLE*M_rob_cell
    
    Whole_blood <- M_art+M_ven
    Lungs <- M_lu_cap + M_lu_is + M_lu_phag+ M_lu_cell
    Rob <- M_rob_cap + M_rob_is + M_rob_cell+M_rob_phag_sinusoids+M_rob_phag_interstital
    ...
    list(c("dM_lu_cap" = dM_lu_cap, "dM_lu_is" = dM_lu_is, 
           ...
           "dM_rob_cell" = dM_rob_cell, "dM_rob_phag_sinusoids" = dM_rob_phag_sinusoids,
           "dM_rob_phag_interstital" = dM_rob_phag_interstital,
...
obj_func <- function(x, dose, df, pars, metric = "AAFE"){
  ...
  parms <- c("rob_pore_size" = exp(x[1]),  "CLE" = exp(x[2]),
             "phago_uptake_rate" =  exp(x[3]),
             "M_lu_max" =  exp(x[4]) , "M_rob_sinusoids_max" =  exp(x[5]),
             "M_rob_interstitial_max" = exp(x[6]),
             "lung_pore_size" = exp(x[7]),
...
# Assume that approximately 10% of the remaining dose is distributed in the carcass
for (i in 1:dim(df)[1]){
  df[i, "rob"] <- df[i, "rob"]+ 0.1*(dose - (df[i, "rob"]+df[i, "blood"]+df[i, "lungs"]))
  df[i, "excreta"] <- 0.9*(dose - (df[i, "rob"]+df[i, "blood"]+df[i, "lungs"]))
...
x0 <-  c(log(10),5,1,1,1,1,log(8))
set.seed(1221312)
optimization<- nloptr::nloptr(x0 = x0,
                              eval_f = obj_func,
                              lb	=  c(log(6.55),-10,-10,-10,-10,-10,log(6.55)),
                              ub =   c(log(2500),20,10,10,10,10,log(20) ),
                              opts = opts,
                              dose = dose,
                              df = df,
                              metric = "SODI",
...
parms <- c("rob_pore_size" = exp(optimization$solution[1]), 
           ...
           "phago_uptake_rate" =  exp(optimization$solution[3]),
           "M_lu_max" =  exp(optimization$solution[4]) ,
           "M_rob_sinusoids_max" =  exp(optimization$solution[5]),
           "M_rob_interstitial_max" =  exp(optimization$solution[6]),
           "lung_pore_size" = exp(optimization$solution[7]),
           "k_rob_in" =  0.051 , 
           "k_rob_out" = 0.063,
