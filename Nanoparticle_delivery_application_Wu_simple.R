...
Wu_model <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dNP_AS <-  -k_AStCV*NP_AS - k_AStOT*NP_AS
    dNP_CV <-  k_AStCV*NP_AS - k_CVtCI*NP_CV - k_CVtOT*NP_CV
    dNP_CI <-  k_CVtCI*NP_CV -(CL_CItEX* NP_CI)/(Km_CItEX+NP_CI)
    dNP_OT <-  k_AStOT*NP_AS+ k_CVtOT*NP_CV- (CL_OTtEX* NP_CI)/(Km_OTtEX+NP_CI)* NP_OT
    dNP_EX <- (CL_CItEX* NP_CI)/(Km_CItEX+NP_CI) + (CL_OTtEX* NP_CI)/(Km_OTtEX+NP_CI)* NP_OT
    
    NP_tot <- NP_AS + NP_CV + NP_OT + NP_CI+dNP_EX
    selectivity <- NP_CI/(NP_AS + NP_CV + NP_OT+NP_CI)
...
case1 <- openxlsx::read.xlsx ('case1.xlsx')
...
parms <- c("k_AStCV" =exp(optimization$solution[1]), 
           "k_CVtCI" = exp(optimization$solution[2]),
           "k_AStOT" = exp(optimization$solution[3]) , 
           "k_CVtOT" = exp(optimization$solution[4]),
           "CL_OTtEX"=  exp(optimization$solution[5]), 
           "Km_OTtEX"=   exp(optimization$solution[6]))
...
ggplot()+
  ...
  geom_point(data = case1, aes(x=time  , y=administration_site ), size=5)+
...
ggplot()+
  ...
  geom_point(data = case1, aes(x=time  , y=cell_vicinity ), size=5)+
...
ggplot()+
  ...
  geom_point(data = case1, aes(x=time  , y=cell_interior ), size=5)+
...
ggplot()+
  ...
  geom_point(data = case1, aes(x=time  , y= off_target ), size=5)+
...
ggplot()+
  ...
  geom_point(data = case1, aes(x=time  , y= excreta ), size=5)+
