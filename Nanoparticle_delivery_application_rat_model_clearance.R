...
Rat_model <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dM_lu_is <- (1-sigma_lu)*QL_lu*C_lu_cap  - k_lu_in *M_lu_is + k_lu_out *M_lu_cell - 
                   M_lu_is*pc_lu - QL_lu*C_lu_is
    dM_lu_cell <- k_lu_in *M_lu_is - k_lu_out *M_lu_cell
    dM_lu_phag <-  M_lu_is*pc_lu 
    
    #Rest of the body
    dM_rob_cap <-  Q_total*C_art - (Q_total-QL_rob)*C_rob_cap - (1-sigma_rob)*QL_rob*C_rob_cap -
                    M_rob_cap*pc_rob_sinusoids
    dM_rob_is <- (1-sigma_rob)*QL_rob*C_rob_cap  - k_rob_in *M_rob_is + k_rob_out *M_rob_cell - 
                   M_rob_is*pc_rob_interstitial - QL_rob*C_rob_is
    dM_rob_cell <- k_rob_in *M_rob_is - k_rob_out *M_rob_cell 
    dM_rob_phag_sinusoids <-  M_rob_cap*pc_rob_sinusoids 
    dM_rob_phag_interstital <-  M_rob_is*pc_rob_interstitial 
    
    # Venous Blood
    dM_ven <-(Q_total-QL_rob)*C_rob_cap + QL_rob*C_rob_is + QL_lu*C_lu_is- Q_total*C_ven- CLE*M_ven
    
    # Arterial Blood
    dM_art <-  (Q_total-QL_lu)*C_lu_cap - Q_total*C_art
    
    # Excreta
    dM_excreta <- CLE*M_ven
...
obj_func <- function(x, dose, df, pars, metric = "AAFE"){
  ...
  parms <- c("rob_pore_size" = exp(x[1]), "lung_pore_size" = exp(x[2]), "CLE" = exp(x[3]),
             "pc_lu" =  exp(x[4]) , "pc_rob_sinusoids" =  exp(x[5]),
             "pc_rob_interstitial" = exp(x[6]),
...
x0 <-  c(log(10),log(8),5,1,1,1,1)
set.seed(435)
optimization<- nloptr::nloptr(x0 = x0,
                              eval_f = obj_func,
                              lb	=  c(log(6.55),log(6.55),-10,-10,-10,-10,-10),
                              ub =   c(log(2500),log(20),20,10,10,10,10 ),
                              opts = opts,
                              dose = dose,
                              df = df,
                              metric = "SODI",
...
parms <- c("rob_pore_size" = exp(optimization$solution[1]), 
           "lung_pore_size" = exp(x[2]),
           "CLE" = exp(optimization$solution[3]),
           "pc_lu" =  exp(optimization$solution[4]) ,
           "pc_rob_sinusoids" =  exp(optimization$solution[5]),
           "pc_rob_interstitial" =  exp(optimization$solution[6]),
           "k_rob_in" =  0.051 , 
           "k_rob_out" = 0.063,
