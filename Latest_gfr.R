...
Rat_model <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dM_rob_cell <- k_rob_in *M_rob_is - k_rob_out *M_rob_cell - CLE_f*M_rob_cell
    ...
    # Arterial Blood
    dM_art <- (Q_total-QL_lu)*C_lu_cap - Q_total*C_art - GFR*CLE_u_frac*C_art
    
    # Excreta
    dM_excreta <- CLE_f*M_rob_cell+GFR*CLE_u_frac*C_art
...
obj_func <- function(x, dose, df, pars, metric = "AAFE"){
  ...
  parms <- c("rob_pore_size" = exp(x[1]), "lung_pore_size" = exp(x[2]),
             "CLE_f" = exp(x[3]),"CLE_u_frac" = exp(x[4]),
             "pc_rob" =  exp(x[5]) ,
             #"k_rob_in" =  exp(x[6]),
           #  "k_rob_out" =  exp(x[7]) ,
...
parms <- c("rob_pore_size" = exp(optimization$solution[1]), 
           ...
           "CLE_u_frac" =  exp(optimization$solution[4]),
