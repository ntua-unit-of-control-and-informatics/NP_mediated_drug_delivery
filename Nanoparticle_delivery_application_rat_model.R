...
create.params <<- function(weight){
  ...
  V_lu_tis <- V_tis[6] 
  V_lu_cap <- V_cap[6] 
  V_rob_tis <- sum(V_tis)-V_tis[6]
  V_rob_cap <- sum(V_cap)-V_cap[6]
  
  return(c("Q_total"=Q_total,  "V_ven"=Vven, "V_art"=Vart,
           "V_lu_tis" = V_lu_tis, "V_rob_tis" = V_rob_tis,
...
create.inits <- function( dose){
  M_lu_cap<-0; M_lu_tis<-0; M_lu_mac <- 0; M_rob_cap<-0; M_rob_tis<-0; M_rob_mac <- 0;
  M_ven <- dose; M_art<-0; M_excreta<-0
    
    return(c( "M_lu_cap" = M_lu_cap, "M_lu_tis"=M_lu_tis,"M_lu_mac"=M_lu_mac,
              "M_rob_cap"=M_rob_cap, "M_rob_tis"=M_rob_tis, "M_rob_mac"=M_rob_mac,
...
Rat_model <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    V_lu_tis <-  parameters["V_lu_tis"]
    V_lu_cap <-  parameters["V_lu_cap"]
    V_rob_tis <- parameters["V_rob_tis"]
    V_rob_cap <- parameters["V_rob_cap"]
    
    # Concentrations (micro grams of NPs)/(mL tissue)
    C_lu_tis <- M_lu_tis/V_lu_tis
    C_lu_cap <- M_lu_cap/V_lu_cap
    C_rob_tis <- M_rob_tis/V_rob_tis
    C_rob_cap <- M_rob_cap/V_rob_cap
    C_ven <- M_ven/V_ven
    C_art <- M_art/V_art

    kluab = k_ab0_lu*(1-(M_lu_mac/M_lu_max))
    krobab = k_ab0_rob*(1-(M_rob_mac/M_rob_max))
    k_de <- 10^(-18)

        # Lungs
    dM_lu_cap <-  Q_total*(C_ven - C_lu_cap) - x_lu*Q_total*(C_lu_cap - C_lu_tis/P_lu)
    dM_lu_tis <-  x_lu*Q_total*(C_lu_cap - C_lu_tis/P_lu)
    dM_lu_mac <-   V_lu_cap*C_lu_cap*kluab - M_lu_mac*k_de
    
    #Rest of the body
    dM_rob_cap <-  Q_total*(C_art - C_rob_cap) - x_rob*Q_total*(C_rob_cap - C_rob_tis/P_rob)
    dM_rob_tis <-  x_rob*Q_total*(C_rob_cap - C_rob_tis/P_rob)- CLE*M_rob_tis
    dM_rob_mac <-   V_rob_cap*C_rob_cap*krobab - M_rob_mac*k_de
    
    # Venous Blood
    dM_ven <- Q_total*C_rob_cap - Q_total*C_ven
    
    # Arterial Blood
    dM_art <-  Q_total*C_lu_cap - Q_total*C_art
    
    # Excreta
    dM_excreta <- CLE*M_rob_tis
    
    Whole_blood <- M_art+M_ven
    Lungs <- M_lu_cap + M_lu_tis + M_lu_mac
    Rob <- M_rob_cap + M_rob_tis + M_rob_mac
    
    list(c("dM_lu_cap" = dM_lu_cap, "dM_lu_tis" = dM_lu_tis,"dM_lu_mac" = dM_lu_mac,
    "dM_rob_cap" = dM_rob_cap, "dM_rob_tis" = dM_rob_tis,"dM_rob_mac" = dM_rob_mac,
...
obj_func <- function(x, dose, df, pars, metric = "AAFE"){
  ...
  parms <- c("P_lu" = exp(x[1]), "P_rob" = exp(x[2]), "CLE" = exp(x[3]),
             "x_lu" = exp(x[4]), "x_rob" =  exp(x[5]),
             "k_ab0_lu" =  exp(x[6]), "k_ab0_rob" =  exp(x[7]),
             "M_lu_max" =  exp(x[8]) , "M_rob_max" =  exp(x[9]),
...
opts <- list( "algorithm" = "NLOPT_LN_SBPLX", #"NLOPT_LN_NEWUOA",NLOPT_LN_SBPLX , #"NLOPT_LN_BOBYQA" #"NLOPT_LN_COBYLA"
  ...
  parms <- c("P_lu" = exp(optimization$solution[1]), "P_rob" = exp(optimization$solution[2]),
             "CLE" = exp(optimization$solution[3]), 
             "x_lu" = exp(optimization$solution[4]),
             "x_rob" = exp(optimization$solution[5]),
             "k_ab0_lu" =  exp(optimization$solution[6]), 
             "k_ab0_rob" =  exp(optimization$solution[7]), 
             "M_lu_max" =  exp(optimization$solution[8]) ,
             "M_rob_max" =  exp(optimization$solution[9]),
             "physiological_pars" = physiological_pars, 
